...
public class OneJumpFallingObject extends FallingObject {
	...
	public OneJumpFallingObject(long tf, float floor, Object2D w) {
		super(R.drawable.airbird, 100, tf, w);
		_jump=0;
		_floor=floor;
		_ball.setY(_floor);
		_only=Place.MIDDLE;
	...
	public void jump() {
		if(_jump==0){
			_world.addObject(new ModelJumpingObject(_ball, _tFall, 400, 430 - _floor, _tFall));
		}
		else{
			...
			_world.addObject(new ModelJumpingObject(_ball, jmp.getTFall(), 350, 430 - _floor, jmp.getTime() - 2 * jmp.getTFall()));				
		}
		++_jump;
	}
	@Override
